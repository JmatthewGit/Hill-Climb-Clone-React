...
function App() {
  ...
  // Physics constants for gravity, friction, acceleration, and max speed
  const gravity = 0.5;
  const friction = 0.98;
  ...
  const updateVehicle = () => {
    // Handle input for left and right key presses
    if (keys.left) {
    ...
    // Limit speed to the maximum value
    ...
    if (vehicle.current.y + Vehicle.height / 2 > terrainY) {
      ...
      vehicle.current.vy = 0; // Stop vertical movement on collision
    }

    // Prevent vehicle from moving off the left or right edge of the canvas
  ...
  // Draw the terrain and the vehicle on the canvas
  const draw = (ctx) => {
    ...
    ctx.rotate(vehicle.current.angle); // Rotate vehicle based on angle
    ctx.fillRect(
      -Vehicle.width / 2,
      -Vehicle.height / 2,
      Vehicle.width,
  ...
  // Get the terrain height (Y position) for a given X coordinate
  const getTerrainY = (x) => {
...
// Generate a simple random terrain for the game
function generateTerrain(width) {
  ...
  for (let x = 0; x < width; x++) {
    const delta = Math.random() * 4 - 2; // Random change between -2 and +2
    y += delta;
